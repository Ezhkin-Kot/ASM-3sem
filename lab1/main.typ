...
MyName db 'Tyumentsev Radomir, 251$' ;Строка для вывода
...
mov DX,offset MyName               ;Адрес начала строки записывается в регистр DX
...
#tracetable("Таблица трассировки программы 3", "table3.csv")
...
Расчёт для SS: $"48ADh" dot "10h" + "0100h" = "48AD0h" + "0100h" = "48BD0h"$. Адрес кратен 16, то есть сегмент DS начинается с адреса 48BD0h. 

Расчёт для DS: $"48BDh" dot "10h" + "0018h" = "48BD0h" + "0018h" = "48BE8h"$. Адрес не кратен 16, то есть сегмент CS начинается с адреса 48BF0h.
...
Изначально сегментный регистры DS и ES указывают на начало префикса программного сегмента PSP, который имеет физический размер в 0100h, и только потом в программе он инициализируется нужным адресом. После него идёт сегмент стека. Физические адреса сегментов выровнены по параграфам, а один параграф содержит 16 байтов, то есть SS смещён относительно DS на 10h.
...
CS: $"48ADh" dot 16 + "0011h" = "48AD0h" + "0011h" = "48AE1h"$. Адрес не кратен 16, округляем вверх: 48AF0h Значит сегмент DS начинается с адреса 48AFh. 

DS: $"48AFh" dot 16 + "0018h" = "48AF0h" + "0018h" = "48B08h"$ Адрес не кратен 16, округляем вверх: 48B10h, значит сегмент SS начинается с адреса 48B1h.
...
В программе 3 (`.model tiny`) регистр SP равен FFFEh, потому что это COM-программа, где стек размещается в самом низу, инициализируясь максимальным значением, и имеет адрес (FFFEh). 
